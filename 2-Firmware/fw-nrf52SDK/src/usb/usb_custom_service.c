...
static nrf_drv_usbd_transfer_t transfer_out1 = {
    .p_data = {.rx = rxBuf},
    .size = 1 + 3,
    .flags = 0};
...
static const uint8_t kbd_report_descriptor[] = {
    ...
    0x09, 0x00, // Usage (0)
    0xA1, 0x01, // Collection (Application)
    0x85, INPUT_REPORT_CUSTOM_ID, // Report ID (3)
    ...
    case NRF_DRV_USBD_EVT_EPTRANSFER:
        ...
        else if (NRF_DRV_USBD_EPOUT1 == p_event->data.eptransfer.ep)
        {
            if (NRF_USBD_EP_OK == p_event->data.eptransfer.status)
            {
                ...
                kb_error(nrf_drv_usbd_ep_transfer(NRF_DRV_USBD_EPOUT1,
                                                  &transfer_out1));                                                ;
                if(rxBuf[0] == OUTPUT_REPORT_KEYBOARD_ID)
                {
                    kb_print("led state:%2x",rxBuf[1]);
                }
                else if(rxBuf[0] == OUTPUT_REPORT_CUSTOM_ID)
                {
            ...
            else if (NRF_USBD_EP_WAITING == p_event->data.eptransfer.status)
            {
                nrf_drv_usbd_ep_transfer(NRF_DRV_USBD_EPOUT1,
                                         &transfer_out1);
            }
            else
            {
...
void usb_hid_send(uint8_t report_id, uint8_t *data)
{
    if (report_id == INPUT_REPORT_KEYBOARD_ID)
    {
        ...
        nrf_drv_usbd_ep_transfer(NRF_DRV_USBD_EPIN1,
                                 &transfer);
    }
